...
int main(void)
{
  ...
  {		 
		...
		for(uint8_t p = 0; p < 48; p++)
	{
		...
		else if(p < 32) scroll_data[p] = led_matrix2[p - 24];
		else if(p < 40) scroll_data[p] = led_matrix3[p - 32];
		else			scroll_data[p] = led_matrix4[p - 40];
	}
	for(uint8_t i=0; i<48; i++)
	{
	  temp = scroll_data[0];
		for(uint8_t p = 0; p < 47; p++)
		scroll_data[p] = scroll_data[p + 1];
		scroll_data[47] = temp;
		matrix_display();
	}
	count++; count %= 10000;
